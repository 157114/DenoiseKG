...
class BaseRetriever(ABC):
    ...
    async def _construct_relationship_context(self, edge_datas: list[dict]):
        ...
        edge_datas = [edge for edge in edge_datas if edge is not None]

        if not edge_datas:
            logger.warning("Edge data was provided, but all entries were invalid (None).")
            return []
        ...
        try:
            ...
            logger.error(f"Error constructing relationship context: {e}")
            return edge_datas
