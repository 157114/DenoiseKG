...
class BaseQuery(ABC):
    ...
    async def _map_global_communities(
        ...
        # Handle case when communities_data is FAIL_RESPONSE string or empty
        if isinstance(communities_data, str) or not communities_data:
            logger.warning(f"No valid community data available: {communities_data}")
            return []
        ...
        logger.info("Creating temporary LLM instance for parallel processing...")
        temp_llm = OpenAILLM(self.llm.config)
