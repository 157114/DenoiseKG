...
class BasicQuery(BaseQuery):
    ...
    async def _retrieve_relevant_contexts_local(self, query):
        ...
        # Guard: if no entities found, return empty contexts to avoid downstream None errors
        if not node_datas:
            logger.warning("No entities retrieved for query; returning empty local contexts")
            empty_entities = list_to_quoted_csv_string([["id", "entity", "type", "description", "rank"]])
            empty_relations = list_to_quoted_csv_string([["id", "source", "target", "description", "weight", "rank"]]) if not self.config.use_keywords else list_to_quoted_csv_string([["id", "source", "target", "keywords", "description", "weight", "rank"]])
            empty_text_units = list_to_quoted_csv_string([["id", "content"]])
            empty_communities = None
            if self.config.use_community:
                empty_communities = list_to_quoted_csv_string([["id", "content"]])
            return empty_entities, empty_relations, empty_text_units, empty_communities
    ...
    async def _retrieve_relevant_contexts_global(self, query):
        ...
        # Check if community_datas is valid before processing
        if isinstance(community_datas, str) or not community_datas:
            logger.warning(f"No valid community data for global search: {community_datas}")
    ...
    async def generation_qa(self, query, context):
        ...
        if self.config.community_information and self.config.use_global_query:
            ...
            # GLOBAL_REDUCE_RAG_RESPONSE uses report_data parameter
            system_msg = sys_prompt_temp.format(
            ...
            # RAG_RESPONSE uses context_data parameter
            system_msg = sys_prompt_temp.format(
            ...
            # LOCAL_RAG_RESPONSE uses context_data parameter
            system_msg = sys_prompt_temp.format(
                context_data=context, response_type=self.config.response_type
            )
        else:
